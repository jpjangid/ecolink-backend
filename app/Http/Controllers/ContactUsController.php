...
class ContactUsController extends Controller
{
    public function index()
    {
        if (checkpermission('ContactUsController@index')) {
            if (request()->ajax()) {
                /* Getting all records */
                $allcontact = DB::table('contact_us')->select('id', 'first_name', 'last_name', 'phone', 'email', 'created_at')->where(['flag' => '0', 'type' => 'contact'])->orderby('created_at','desc')->get();
                ...
                return Datatables::of($contacts)
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        <form action="' . $delete_url . '" method="post">
                         ...
                         <button class="delete btn btn-danger btn-xs contact_confirm"><i class="fas fa-trash"></i></button>
                      </form>
                     </div>';
                        return $btn;
                    })
    ...
    {
        if (checkpermission('ContactUsController@edit')) {
    ...
    {
        if (checkpermission('ContactUsController@destroy')) {
